...
const NewUserDiscounts = ({ copyPromoCode, copiedCode, offers }) => {
  ...
  const welcomeCode = welcomeOffer?.code || 'HELLO50';
  const welcomeTitle = welcomeOffer?.type === 'PERCENTAGE' && welcomeOffer.discountPercentage ? `${welcomeOffer.discountPercentage}% OFF` : (welcomeOffer?.title || '50% OFF');
  const welcomeDesc = welcomeOffer?.description || 'On your first 3 orders. Max discount $15 per order. Use code:';
  const welcomeImg = welcomeOffer?.image && welcomeOffer.image !== 'no-photo.jpg' ? welcomeOffer.image : "https://lh3.googleusercontent.com/aida-public/AB6AXuDhjBa9WNpwPTWCPb115FVDOrmq5keeqSge7gVzZqe_Z1yDHMgInyBleGat0YJw9o_mH8tZiFRSjl__7MizThqNKIJrjOC2dky6e4UHamh3lkv4X2raM1Y_5cj8GJWHq6jGCc2nPpb5ewcUxNBUErRRK8lb4Gz-TU2R4cxJf9IcnFH6AEnh3zjPykA6xJPd0Vs7C0vlVRGNZWMv_OyeqkITlR6EUtelNlKnTCWrw-LQCrB1XML-Wjc_2A";

  const deliveryCode = deliveryOffer?.code || 'FREEDELIVERY';
  const deliveryTitle = deliveryOffer?.title || 'FREE Delivery';
  const deliveryDesc = deliveryOffer?.description || "No minimum order for your first month. Stay hungry, we've got the gas.";
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-gutter h-auto lg:h-[400px]">
        ...
        <div className="md:col-span-8 bg-surface-container-low border border-outline-variant/30 rounded-[24px] p-8 flex flex-col justify-between relative overflow-hidden group shadow-sm">
          ...
          <div className="relative z-10 flex gap-4 mt-6">
            <button
              onClick={() => copyPromoCode(welcomeCode)}
            ...
            >
              ...
              {copiedCode === welcomeCode ? 'Copied!' : 'Claim Code'}
            </button>
          </div>
          <div className="absolute right-0 bottom-0 w-1/2 h-full hidden lg:block">
            <img
              ...
              alt="Welcome pack mockup"
              src={welcomeImg}
            />
          </div>
        </div>
        {/* Right Bento: Delivery Trial */}
        <div className="md:col-span-4 bg-primary text-white rounded-[24px] p-8 flex flex-col justify-center items-center text-center relative overflow-hidden shadow-sm">
          ...
          <h3 className="text-h3 font-h3 mb-2 font-bold">{deliveryTitle}</h3>
