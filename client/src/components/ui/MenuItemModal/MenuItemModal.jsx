...
const MenuItemModal = ({ item, isOpen, onClose, onAddToCart }) => {
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-end md:items-center justify-center animate-in fade-in">
      ...
      <div className="relative w-full md:w-[500px] max-h-[90vh] bg-white rounded-t-3xl md:rounded-3xl shadow-2xl flex flex-col animate-in slide-in-from-bottom-10 md:zoom-in-95">
            ...
            <div className="mb-6">
              ...
              <div className="space-y-3">
                {item.sizes.map((size, idx) => (
                  <label key={idx} className="flex justify-between items-center p-3 border rounded-xl cursor-pointer hover:border-primary transition-colors has-[:checked]:border-primary has-[:checked]:bg-primary/5">
                    <div className="flex items-center gap-3">
                      ...
                      />
                      <span className="font-body font-bold">{size.name}</span>
                    </div>
        ...
        {/* Action Footer */}
        <div className="absolute bottom-0 left-0 right-0 p-4 bg-white border-t border-surface-variant md:relative md:border-t-0 md:pt-0">
          <button 
