...
const FlashSaleBanner = ({ formattedTime, copyPromoCode, copiedCode, offer }) => {
  ...
  const title = offer?.title || "Today's Top Deals";
  const description = offer?.description || "Satisfy your cravings for less. Exclusive discounts on the city's finest restaurants, only for the next few hours.";
  const promoCode = offer?.code || 'FLASH80';
  const discountText = offer?.type === 'PERCENTAGE' && offer.discountPercentage 
  ...
  return (
    <section className="relative w-full rounded-[32px] overflow-hidden mb-stack_lg min-h-[400px] flex items-center shadow-md">
      ...
      <div className="relative z-10 p-8 md:p-16 max-w-2xl text-white">
        ...
        <h1 className="font-h1 text-h1-mobile md:text-h1 mb-4 leading-tight">{title}</h1>
